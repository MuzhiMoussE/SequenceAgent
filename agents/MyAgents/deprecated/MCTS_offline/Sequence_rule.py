...
class SequenceGameRule(GameRule):
    def __init__(self, num_of_agent, ownid):
        super().__init__(num_of_agent)
        self.private_information = [
            "hand"]  # Private information is a list of agent attributes that cannot be known between agents.
        self.ownid = ownid
    ...
    def generateSuccessor(self, state, action, agent_id):
        ...
        card = action_dict['play_card']
        draft = action_dict['draft_card']
        if card:
        ...
        if action_dict['type'] == 'trade':
            plr_state.trade = True  # Switch trade flag to prohibit agent performing a second trade this turn.
            plr_state.agent_trace.action_reward.append((action_dict, reward))  # Log this turn's action and score (zero).
            return state

        # Update Sequence board. If action was to place/remove a marker, add/subtract it from the board.
        r, c = action_dict['coords']
        if action_dict['type'] == 'place':
            state.board.chips[r][c] = plr_state.colour
            state.board.empty_coords.remove(action_dict['coords'])
            state.board.plr_coords[plr_state.colour].append(action_dict['coords'])
        elif action_dict['type'] == 'remove':
            state.board.chips[r][c] = EMPTY
            state.board.empty_coords.append(action_dict['coords'])
            state.board.plr_coords[plr_state.opp_colour].remove(action_dict['coords'])
        else:
                ...
                for sequence in seq['coords']:
                    for r, c in sequence:
                        if state.board.chips[r][c] != JOKER:  # Joker spaces stay jokers.
                            ...
                            try:
                                state.board.plr_coords[plr_state.colour].remove(action_dict['coords'])
        ...
        plr_state.agent_trace.action_reward.append((action_dict, reward))  # Log this turn's action and any resultant score.
    ...
    def isEnds(self, checkstate):  # Game ends if a team has formed at least 2 sequences, or if the deck is empty.
        score_check=0
        score_check += checkstate.agents[self.ownid].completed_seqs
        return score_check>=2 or len(checkstate.board.draft) == 0
    ...
    def execute(self, receive_state, action):
        possible_state = self.generateSuccessor(receive_state, action, self.ownid)
        new_score = possible_state.agents[self.ownid].score
        score_check = 0
        score_check += possible_state.agents[self.ownid].completed_seqs
        return possible_state, new_score, (score_check>=2 or len(possible_state.board.draft) == 0)
